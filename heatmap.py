...
# ---------- CLI ----------
def parse_args():
    ap = argparse.ArgumentParser("EdgeYOLO TensorRT 10.13 + Video (MP4) — detection + density + stats")
    ...
    ap.add_argument("--save", default="output_detection.mp4", type=str, help="Output video path")

    # Yeni eklenenler
    ap.add_argument("--density-metric", choices=["max", "mean"], default="max",
                    help="Density score: 'max' pixel intensity or 'mean' intensity")
    ap.add_argument("--sigma", type=float, default=25,
                    help="Sigma value for Gaussian blur in heatmap")
...
def main():
    ...
    frame_idx = -1
    counts_over_time = []  # kişi sayısını sakla
    ...
    while True:
        ...
        if out.shape[1] == 6:
            ...
            # xyxy back to original frame coords
            xyxy[:, [0, 2]] -= dw   # x1, x2
            xyxy[:, [1, 3]] -= dh   # y1, y2
            xyxy /= r
        ...
        for i in range(len(xyxy)):
            ...
            cv2.putText(frame, f"{conf[i]:.2f}", (x1, max(0, y1 - 5)),
        ...
        heatmap = cv2.GaussianBlur(heatmap, (0, 0), args.sigma)

        # --- Density metric ---
        if args.density_metric == "max":
            raw_density_score = float(heatmap.max())
        else:  # mean
            raw_density_score = float(heatmap.mean())

        # sadece görselleştirme için normalize et
        if heatmap.max() > 0:
            heatmap_vis = (heatmap / heatmap.max() * 255).astype(np.uint8)
        else:
        ...
        # --- Metrics ---
        people_count = len(xyxy)
        counts_over_time.append(people_count)

        cv2.putText(overlay, f"Count: {people_count}", (20, 40),
                    cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2)
        cv2.putText(overlay, f"Density({args.density_metric}): {raw_density_score:.2f}", (20, 80),
                    cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2)

        # --- Simple graph (bottom 100px) ---
        graph_h = 100
        graph = np.zeros((graph_h, overlay.shape[1], 3), dtype=np.uint8)
        if len(counts_over_time) > 1:
            max_count = max(counts_over_time)
            if max_count == 0: max_count = 1
            scale_y = graph_h / max_count
            for i in range(1, len(counts_over_time)):
                x1 = int((i-1) * overlay.shape[1] / len(counts_over_time))
                y1 = graph_h - int(counts_over_time[i-1] * scale_y)
                x2 = int(i * overlay.shape[1] / len(counts_over_time))
                y2 = graph_h - int(counts_over_time[i] * scale_y)
                cv2.line(graph, (x1, y1), (x2, y2), (0, 255, 0), 2)
        overlay = np.vstack((overlay, graph))

        # Writer
        if out_writer is None:
            h_out_v, w_out_v = overlay.shape[:2]
        ...
        # Show
        disp_frame = cv2.resize(overlay, (1280, 720), interpolation=cv2.INTER_LINEAR)
